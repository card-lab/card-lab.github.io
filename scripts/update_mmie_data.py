...
refresh the statistics and highlights in `files/site/_mmie_data.yml`.
...
The script will:
  ...
  2. Send the scraped text to Claude with a prompt asking it to extract
    ...
    3. Write the result back to `files/site/_mmie_data.yml`.
...
OUTPUT_PATH = ROOT_DIR / "files" / "site" / "_mmie_data.yml"
...
def main():
    ...
    header = (
        ...
        "# Edit manually OR re-run:  python scripts/update_mmie_data.py\n\n"
    )
    ...
    print("  Re-render the site with:  quarto render capabilities/mmie-research.qmd")
