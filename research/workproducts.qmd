...
include-in-header:
    text: |
        ...
        // Set loading message dynamically based on cache source
        document.addEventListener('DOMContentLoaded', () => {
            document.getElementById('loadingMessage').innerHTML = `<em>Please wait. Loading content from the cached CARD group library...</em>`;});
        </script>
---
...
<script>
    ...
    'map','newspaperArticle','patent','podcast','preprint','presentation','radioBroadcast','report','statute',
    ...
    newspaperArticle: 'Newspaper Article', patent: 'Patent', podcast: 'Podcast', preprint: 'Preprint',
    presentation: 'Presentation', radioBroadcast: 'Radio Broadcast', report: 'Report', statute: 'Statute', thesis: 'Thesis',
    tvBroadcast: 'TV Broadcast', videoRecording: 'Video Recording', webpage: 'Web Page'
    };
    
    async function fetchZoteroItems() {
    try {
        const localResponse = await fetch('../files/zotero-group-library.json');
        if (!localResponse.ok) {
            throw new Error(`Cache fetch failed: ${localResponse.status} ${localResponse.statusText}`);
        }
        return localResponse.json();
    } catch (cacheError) {
        console.warn('Falling back to live Zotero API:', cacheError.message);
        // The Zotero API caps each response at 100 items regardless of the
        // requested limit, so a single fetch silently truncates the library.
        // Page through with start/limit until a short page signals the end.
        const pageSize = 100;
        let start = 0;
        const allItems = [];
        while (true) {
            const response = await fetch(`https://api.zotero.org/groups/${ZOTERO_GROUP_ID}/items?format=json&limit=${pageSize}&start=${start}`);
            if (!response.ok) throw new Error(`Error fetching live data: ${response.statusText}`);
    ...
    function formatCreatorName(creator) {
        ...
        if (creator.firstName && creator.lastName) {
            return `${creator.firstName.split(/\s+/).map(name => name.charAt(0).toUpperCase() + '.').join(' ')} ${creator.lastName}`;
        }
    ...
    function formatAuthors(creators) {
        if (!creators) return '';
        return creators.map(formatCreatorName).filter(name => name.trim() !== '').join(', ');
    }

    function formatPresentationCreators(creators) {
        if (!creators) return '';
        return creators.map((creator) => {
            const name = formatCreatorName(creator);
            if (!name.trim()) return '';
            if (creator.creatorType === 'presenter') {
                return `<span class="italic">${name}</span> (presenter)`;
            }
            return name;
    ...
    function formatCitation(item, index, total) {
        ...
        const formattedAuthors = d.itemType === 'presentation'
            ? formatPresentationCreators(mainContributors)
            : formatAuthors(mainContributors);
        ...
        const fullDate = (d.date || '').toString().trim();
        const isManuscriptLike = d.itemType === 'manuscript' || d.itemType === 'preprint';
        ...
        if (isManuscriptLike) { if (d.place) citation += `, ${d.place}`; if (fullDate) citation += ` (${fullDate})`; }
    ...
    const filtered = zoteroItems.filter(item => {
        ...
        ? (typeVal === 'manuscript'
            ? (d.itemType === 'manuscript' || d.itemType === 'preprint')
            : d.itemType === typeVal)
        : true;
